...
try {
  const { values, positionals } = parseArgs({ allowPositionals: true, options: { home: { type: 'string' }, project: { type: 'string' }, role: { type: 'string' }, resume: { type: 'boolean' }, 'quick-commands': { type: 'boolean' }, 'shell-profile': { type: 'string' }, help: { type: 'boolean' } } });
  const action = positionals[0] || 'start';
  if (values.help) {
    console.log('orca-team [init|start|status|install|export-config] [--project PATH] [--home PATH]\ninstall --quick-commands also registers Orca global shortcuts. No provider configuration is modified.');
  } else {
    ...
    if (action === 'install') {
      ...
      const saved = values.resume ? readJson(projectFiles(home, project).state).agents[role.name] : null;
